...
months_sim = 36; % Number of months to run simulation over
...
H_growth = 1e-2; % Human population growth from feeding on sharks
...
dYdt_a = @(Yt, Pt, Ht, t) Y_grow * Yt - Y_prey_P * (Pt * Yt);

% Change in tuna population
dYdt_b = @(Yt, Pt, Ht, t) Y_grow * Yt - Y_prey_P * (Pt * Yt) ...
   - P_fish * (Ht * Yt);

points = 1:num_points;
times = points * dt;

% Logarithmically spaced bounds to test
myMin = -3;
myMax = -1;

% Graphs to make for each model
num_plots = 5;
...
for i = 1:2
    ...
    for H_growth = logspace(myMin, myMax, num_plots)
        % Change in human population
        dHdt = @(Yt, Pt, Ht, t) - H_death * Ht + H_growth * (Ht * Pt); 
        ...
        title(strcat('Human growth =', num2str(H_growth)))
        xlabel('Time (times)')
        ylabel('Population')
        legend('show')
    end
