...
hold on;
title('CPU Temperature Over Time, KE inc = 100')
xlabel('Time (seconds)');
ylabel('Kinetic energy');

% Test different
for init_n = 10:25:100
    
    % Initial numbers of particles
    % Number of particles inside case
    init_n_inside = round(init_n / 2);
    
    % Number of particles outside of case
    init_n_outside = round(init_n / 2);
    ...
    air_density = count_assigned / (canvas_width * canvas_height);
    plot(times, cpu_temps(1, :), 'DisplayName', ...
        sprintf('Air density: %1.3f num / A', air_density));
end
legend;
