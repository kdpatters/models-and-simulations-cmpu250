...
output_vid = true; % Alternative is to render in real time
...
outside = @(x, y) or(or(y > top_side_y, y < bottom_side_y), ...
            or(y < left_side_x, x > right_side_x));
        
for i = 1:n_walls
    wall(i).therm_cond = 1; % Make the walls thermally conductive
    wall(i).handle = i;
...
hold on;
for k = 1:length(wall)
    plot(times, wall_temps(k, :));
end
title('Wall Temperatures Over Time')
xlabel('Time (seconds)');
ylabel('Temperature');
...
legend('Number inside', 'Number outside');
